...
if (random == SCISSORS && paperSelected) { 
 ...
 message = "";
  ...
  if (localStorage.CookieCountGoUp >= 10){
    if(localStorage.wins >= localStorage.lost && localStorage.wins >= localStorage.ties )
      other = " Congrats you won!"

  else if ( localStorage.lost >= localStorage.wins && localStorage.lost >= localStorage.ties )
    other = " Thats to bad you lost try again?"
    else if ( localStorage.ties >= localStorage.wins && localStorage.ties >= localStorage.lost)
  ...
  document.getElementById("win").innerHTML = "you have won " + localStorage.wins + " times"
   ...
   document.getElementById("game").innerHTML = " You have played " + localStorage.great + " Games against the bot "
